  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between border border-gray-200 rounded-xl px-4 py-2.5 bg-white text-sm text-gray-900 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
      >
        <span>{label}</span>
        <ChevronDown size={15} className={`text-gray-400 transition-transform duration-150 ${open ? 'rotate-180' : ''}`} />
      </button>
      ...
      {open && (
        <div className="absolute z-50 mt-1.5 w-full bg-white border border-gray-100 rounded-xl shadow-lg overflow-hidden">
          {normalised.map(o => (
            ...
            >
              <span className={o.value === value ? 'font-medium text-indigo-600' : 'text-gray-700'}>
                {o.label ?? o.value}
              </span>
              {o.value === value && <Check size={14} className="text-indigo-600" />}
            </button>
          ))}
        </div>
      )}
